...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600 mt-1">Transaksi restoran dari {{ \Carbon\Carbon::parse($from)->format('d M Y') }} - {{ \Carbon\Carbon::parse($to)->format('d M Y') }}</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('owner.laporan.print', ['from' => $from, 'to' => $to]) }}" target="_blank"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ optional($t->created_at)->format('d M Y H:i') ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">{{ strtoupper($t->metode_pembayaran ?? '-') }}</td>
                    </tr>
                    @empty
                    <tr>
                ...
                <tfoot class="bg-gray-50">
                    <tr>
                        ...
                        <th colspan="4" class="px-6 py-3 text-left text-sm font-bold text-gray-900">Rp {{ number_format($totalOmset ?? 0, 0, ',', '.') }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
